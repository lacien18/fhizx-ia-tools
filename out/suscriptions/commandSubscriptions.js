...
function registerCommands(context, fileManager, refreshAll) {
    const categories = [
        "prompts",
        "agents",
        "skills",
    ...
    context.subscriptions.push(vscode.commands.registerCommand("fhizxAiTools.sendToChat", async (node) => {
        let filePath = "";
        ...
        try {
            ...
            vscode.window.showInformationMessage(`El contenido de "${fileName}" se copió al portapapeles.`);
        }
    ...
    }), vscode.commands.registerCommand("fhizxAiTools.openFile", (uri) => vscode.window.showTextDocument(uri)), 
    // Generadores dinámicos para comandos específicos
    ...categories.flatMap((cat) => [
        vscode.commands.registerCommand(`fhizxAiTools.create${cat.charAt(0).toUpperCase() + cat.slice(1, -1)}File`, () => fileManager.createNewFile(cat, refreshAll)),
        vscode.commands.registerCommand(`fhizxAiTools.create${cat.charAt(0).toUpperCase() + cat.slice(1, -1)}Folder`, () => fileManager.createNewFolder(cat, refreshAll)),
    ]), vscode.commands.registerCommand("fhizxAiTools.createFileContext", (node) => fileManager.createNewFile("prompts", refreshAll, node)), vscode.commands.registerCommand("fhizxAiTools.createFolderContext", (node) => fileManager.createNewFolder("prompts", refreshAll, node)), vscode.commands.registerCommand("fhizxAiTools.renameItem", async (node) => {
        if (!node)
            return;
...
//# sourceMappingURL=commandSubscriptions.js.map